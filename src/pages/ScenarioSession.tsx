...
export default function ScenarioSession() {
    ...
    useEffect(() => {
        ...
        async function startAvatar() {
            ...
            try {
                ...
                const origSend = (
                    s.sendCommandEvent as (...a: unknown[]) => void
                ).bind(session);
                s.sendCommandEvent = (cmd: {
                    event_type: string;
                    text?: string;
                }) => {
                ...
                session.on(SessionEvent.SESSION_STATE_CHANGED, (state) => {
                    if (state === SessionState.CONNECTED) {
                        const { persona } = scenario!;
                        const facts = persona.implicitFacts
                            .map((f, i) => `${i + 1}. ${f}`)
                            .join(" ");
                        session.message(
    ...
    function stopListening() {
        ...
        buffer.forEach((e) =>
            setTranscript((prev) => [...prev, { role: "user", ...e }]),
        );
    ...
    function handleMicClick() {
        if (
            isAvatarTalkingRef.current ||
            !isAvatarReadyRef.current ||
            isPendingResponseRef.current
        )
            return;
            ...
            <Box sx={{ display: "flex", height: "calc(100vh - 96px)" }}>
                ...
                >
                    ...
                    >
                        ...
                        {scenario.persona.scenarioDescription}
                    </Typography>
                            ...
                            <Typography
                                ...
                                sx={
                                    showListening
                                        ? {
                                              animation:
                                                  "listenPulse 4s ease-in-out infinite",
